...
public class Graph<T> implements GraphADT<T> {
    ...
    public void removeVertex(int index) {
        ...
        if (!indexIsValid(index)) {
            ...
            // decrement numVertices
            numVertices--;

            // remove vertex, shift remaining of array items left
            for (int i = index; i < numVertices; i++) {
                vertices[i] = vertices[i + 1];
            }

            // shift adjMatrix left after index
            for (int i = 0; i < numVertices; i++) {
            ...
            // shift adjMatrix up after index
            for (int i = index; i < numVertices; i++) {
    ...
    {
        // for each vertex, do BF traversals.
        // check if result equals numVertices
        for(int i=0; i<numVertices; i++){
            int count=0;
            Iterator it = iteratorBFS(vertices[i]);
            while(it.hasNext()){
                it.next();
                count++;
